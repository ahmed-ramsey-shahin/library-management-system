...
{
    public sealed class GetBooksByTitleQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetBooksByTitleQuery, Result<PaginatedList<BookSummaryDto>>>
    {
        public async Task<Result<PaginatedList<BookSummaryDto>>> Handle(GetBooksByTitleQuery request, CancellationToken cancellationToken)
        {
            var searchTerm = $"%{request.Title}%";
            var booksQuery = db.Books
                .AsNoTracking()
                .Where(book => EF.Functions.Like(book.Title, searchTerm));
