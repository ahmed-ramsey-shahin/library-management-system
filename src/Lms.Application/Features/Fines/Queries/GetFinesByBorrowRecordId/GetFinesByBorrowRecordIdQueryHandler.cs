...
{
    public sealed class GetFinesByBorrowRecordIdQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetFinesByBorrowRecordIdQuery, Result<PaginatedList<FineDto>>>
    {
        public async Task<Result<PaginatedList<FineDto>>> Handle(GetFinesByBorrowRecordIdQuery request, CancellationToken cancellationToken)
        {
            var finesQuery = db.Fines
                .AsNoTracking()
                .Where(fine => fine.BorrowRecordId == request.BorrowRecordId);
