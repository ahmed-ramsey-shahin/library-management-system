...
{
    public sealed class GetMemberFinesQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetMemberFinesQuery, Result<PaginatedList<FineDto>>>
    {
        public async Task<Result<PaginatedList<FineDto>>> Handle(GetMemberFinesQuery request, CancellationToken cancellationToken)
        {
            var finesQuery = db.Fines
                .AsNoTracking()
                .Where(fine => fine.MemberId == request.MemberId);
