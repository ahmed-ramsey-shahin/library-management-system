...
namespace Lms.Application.Features.Audiences.Commands.DeleteAudience
{
    ...
    ) : IRequestHandler<DeleteAudienceCommand, Result<Deleted>>
    {
        ...
        {
            var keyword = await db.Audiences.FirstOrDefaultAsync(keyword => request.AudienceId == keyword.Id, cancellationToken);
            ...
            {
                logger.LogInformation("Deleted keyword {AudienceId}", request.AudienceId);
            }
