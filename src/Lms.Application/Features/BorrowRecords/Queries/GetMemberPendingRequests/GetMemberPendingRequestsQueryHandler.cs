...
{
    public sealed class GetMemberPendingRequestsQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetMemberPendingRequestsQuery, Result<PaginatedList<BorrowRecordSummaryDto>>>
    {
        public async Task<Result<PaginatedList<BorrowRecordSummaryDto>>> Handle(GetMemberPendingRequestsQuery request, CancellationToken cancellationToken)
        {
            var borrowRecordsQuery = db.BorrowRecords
                .AsNoTracking()
                .Where(record => record.MemberId == request.MemberId && record.Status == BorrowRecordStatus.Waiting);
