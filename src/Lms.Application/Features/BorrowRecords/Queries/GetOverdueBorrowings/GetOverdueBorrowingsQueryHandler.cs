...
namespace Lms.Application.Features.BorrowRecords.Queries.GetOverdueBorrowings
{
    public sealed class GetOverdueBorrowingsQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetOverdueBorrowingsQuery, Result<PaginatedList<BorrowRecordSummaryDto>>>
    {
        public async Task<Result<PaginatedList<BorrowRecordSummaryDto>>> Handle(GetOverdueBorrowingsQuery request, CancellationToken cancellationToken)
        {
            ...
            var borrowRecordsQuery = db.BorrowRecords
                ...
                .Where(
                    ...
                    today > record.DueDate &&
                    (record.Status == BorrowRecordStatus.Late || record.Status == BorrowRecordStatus.Accepted)
                );
