...
{
    public sealed class GetMemberActiveBorrowingsQueryHandler(
        IAppDbContext db
    ) : IRequestHandler<GetMemberActiveBorrowingsQuery, Result<PaginatedList<BorrowRecordSummaryDto>>>
    {
        public async Task<Result<PaginatedList<BorrowRecordSummaryDto>>> Handle(GetMemberActiveBorrowingsQuery request, CancellationToken cancellationToken)
        {
            var borrowRecordsQuery = db.BorrowRecords
                .AsNoTracking()
                .Where(record => record.MemberId == request.MemberId && (record.Status == BorrowRecordStatus.Accepted || record.Status == BorrowRecordStatus.Late));
