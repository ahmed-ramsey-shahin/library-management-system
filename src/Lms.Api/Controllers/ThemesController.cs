...
using Lms.Application.Features.Themes.Queries.GetThemesByBookId;
...
namespace Lms.Api.Controllers
{
    ...
    public class ThemesController(ISender sender) : ApiController
    {
        ...
        [MapToApiVersion("1.0")]
        [ProducesDefaultResponseType]
        public async Task<IActionResult> Create([FromHeader(Name="X-Idempotency-Key")] string idempotencyKey, [FromBody] CreateThemeRequest request, CancellationToken cancellationToken)
        {
            var result = await sender.Send(new CreateThemeCommand(request.Name, idempotencyKey), cancellationToken);
        ...
        [MapToApiVersion("1.0")]
        [ProducesDefaultResponseType]
        public async Task<IActionResult> Update(Guid themeId, [FromBody] CreateThemeRequest request, CancellationToken cancellationToken)
        {
            var result = await sender.Send(new UpdateThemeCommand(themeId, request.Name), cancellationToken);
            return result.Match(_ => NoContent(), Problem);
        }

        [HttpGet("/api/v{version:apiVersion}/books/{bookId:guid}/themes")]
        [ProducesResponseType(typeof(List<ThemeDto>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [MapToApiVersion("1.0")]
        [EndpointName("GetThemesByBook")]
        public async Task<IActionResult> GetThemesByBook(Guid bookId, CancellationToken cancellationToken)
        {
            var result = await sender.Send(new GetThemesByBookIdQuery(bookId), cancellationToken);
            return result.Match(Ok, Problem);
        }
    }
}
